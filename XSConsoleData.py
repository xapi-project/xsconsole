...
class Data:
    ...
    def Update(self):
        ...
        if self.session is not None:
            try:
                ...
                self.data['pools'] = {}
                for id, pool in list(pools.items()):
            ...
            try:
                ...
                for opaqueRef, values in list(srMap.items()):
    ...
    def ReadTimezones(self):
        ...
        filterExp = re.compile('('+'|'.join(list(self.data['timezones']['continents'].values()))+')')
    ...
    def ReadKeymaps(self):
        ...
        for value in list(self.data['keyboard']['namestomaps'].values()):
    ...
    def KeymapToName(self, inKeymap):
        ...
        for key, value in list(self.keyboard.namestomaps({}).items()):
            if value == inKeymap:
                mapName = key
    ...
    def ManagementNetmask(self, inDefault = None):
        ...
        for pif in self.derived.managementpifs([]):
            if pif['ip_configuration_mode'].lower().startswith('static'):
                ...
                ifconfig = subprocess.getoutput("/sbin/ifconfig '"+device+"'").split("\n")
    ...
    def ManagementGateway(self, inDefault = None):
        ...
        for pif in self.derived.managementpifs([]):
            if pif['ip_configuration_mode'].lower().startswith('static'):
                ...
                routes = subprocess.getoutput("/sbin/route -n").split("\n")
                for line in routes:
    ...
    def PurgeVBDs(self):
        ...
        for vbd in list(vbdRefs.values()):
            try:
